...
export const recordHoleScore = async (req, res) => {
    // ============================================================
    ...
    const holeSpec = await prisma.hole.findUnique({
    ...
    const savedScore = await prisma.score.upsert({
        ...
        where: {
            score_id: await prisma.score.findFirst({
                where: {
    ...
    if (toParResult === 0)
        toParDisplay = "E"; // Even Par (เสมอตัว)
    if (toParResult > 0)
        toParDisplay = `+${toParResult}`; // เกินพาร์ (เช่น +1, +2)
    // ============================================================
...
//# sourceMappingURL=scorer.tournament.controller.js.map